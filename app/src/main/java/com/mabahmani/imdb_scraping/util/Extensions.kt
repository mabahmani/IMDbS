...
fun Context.showUnexpectedError(retry: () -> Unit){
    MaterialAlertDialogBuilder(this)
        .setTitle(resources.getString(R.string.unexpected_error))
        .setMessage(resources.getString(R.string.unexpected_error_suggest))
        .setPositiveButton(resources.getString(R.string.try_again)) { _, _ ->
            retry.invoke()
        }
        .setCancelable(false)
